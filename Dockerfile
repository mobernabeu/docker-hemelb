...
RUN mkdir hemelb/dependencies/build && \
    ...
    mkdir build && \
    cd build && \
    cmake -DHEMELB_STEERING_LIB:string=none -DHEMELB_KERNEL:string=NNCYMOUSE -DHEMELB_USE_SSE3:string=ON -DHEMELB_OPTIMISATION:string="-O3 -DNDEBUG" -DHEMELB_WALL_BOUNDARY:string=BFL -DHEMELB_INLET_BOUNDARY:string=LADDIOLET -DHEMELB_WALL_INLET_BOUNDARY:string=NASHZEROTHORDERPRESSUREBFL -DHEMELB_WALL_OUTLET_BOUNDARY:string=NASHZEROTHORDERPRESSUREBFL .. && \
    make install
